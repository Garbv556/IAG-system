...
class WoolooAgent(AnimalAgent):
    def __init__(self, agent_id: str = "wooloo_001"):
        super().__init__(agent_id=agent_id)
        # O construtor chama _set_animal_characteristics internamente.
        
    def _set_animal_characteristics(self) -> None:
        self.animal_name = "Wooloo"
        self.species = "Pokémon"
        self.environment = "Mundo Pokémon"
        self.characteristics = ["coesão"]
        # Adicionamos "powers" dinamicamente no dict ou na base se quiser
        self.powers = ["resistência simples"]
        
        self.solo_behavior = "evite risco"
        self.group_behavior = "mantenha união"
        self.rival_response = "recuo coletivo"
        
        # ═══ DOUTRINA TÁTICA ═══
        self.solo_tactics = ['evite risco']
        self.collective_tactics = ['mantenha união']
        self.threat_tactics = ['recuo coletivo']
